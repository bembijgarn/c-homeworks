...
--1
Select c.contactname,c.country,o.orderdate from Sales.Customers c
join Sales.orders o on c.custid = o.custid
--2
Select Upper(prod.productname) as productname,prod.unitprice,cat.categoryname from Production.Products prod
join Production.Categories cat on cat.categoryid = prod.productid where prod.unitprice between 20 and 40
--3
Select em.lastname,em.firstname,o.orderid,o.freight from HR.Employees em
join Sales.Orders o on o.empid = em.empid where o.freight > 50 AND em.title = 'Sales Manager'
--4
Select o.orderdate,cu.contactname,cu.country,cu.address from Sales.Customers cu
join Sales.Orders o on o.shipcity = cu.city  where o.orderdate = '2007'
--5
Select distinct o.shipcity from Sales.Orders o
join HR.Employees e on e.empid = o.empid where e.lastname = 'Cameron'
--6
Select s.companyname,o.orderid,o.shipcountry,o.shipcity from Sales.Orders o
join Sales.Shippers s on s.shipperid = o.shipperid 
where o.shipcountry in ('Germany','Austria')
--7
Select p.productid,p.productname,p.supplierid,p.categoryid,p.unitprice,p.discontinued from Production.Products p
join Production.Suppliers s on s.supplierid = p.supplierid 
where s.city = 'Tokyo' AND p.discontinued = '1'
--8
Select pr.productname,cat.categoryname from Production.Products pr
join Production.Categories cat on cat.categoryid = pr.categoryid 
join Production.Suppliers sup on pr.supplierid = sup.supplierid
where sup.country = 'Japan'
--9
Select emp.firstname,emp.lastname,sh.companyname from HR.Employees emp
join Sales.Orders o on o.empid = emp.empid
join Sales.Shippers sh on o.shipperid = sh.shipperid
where emp.lastname in ('Davis','Cameron')
--10
Select pr.productname,cat.categoryname from Production.Products pr
join Production.Categories cat on cat.categoryid = pr.categoryid
join Production.Suppliers sup on pr.supplierid = sup.supplierid
...
--11
Select o.orderid,emp.lastname,emp.firstname,emp.city,cu.contactname from Sales.Orders o
join HR.Employees emp on o.empid = emp.empid
join Sales.Customers cu on cu.custid = o.custid
where cu.city  = emp.city
--12
Select distinct cu.contactname from Sales.Customers cu 
join Sales.Orders o on cu.custid = o.custid
join Sales.OrderDetails ord on ord.orderid = o.orderid
join Production.Products prod on prod.productid = ord.productid
join Production.Categories catz on catz.categoryid = prod.categoryid
where catz.categoryname in ('Beverages','Dairy Products')